...
|---|---|---|
| Count total edges | ≤8 (single-column), ≤10 (single-row), ≤15 (multi-column grid) | Consolidate parallel edges or split into multiple diagrams |
| Count parallel edges (same source type → same target type, same semantic) | If ≥3 parallel edges of same meaning | Consolidate into 1 edge with multiplicity label: `{label: "archive (3x)"}` |
| Check for layer-skipping edges | Any edge connecting non-adjacent layers | Add pass-through routing nodes in intermediate layers, or use bus notation |
| Check flow type count | If >2 flow types share a vertical corridor | Spatially separate: write path left, read path right, shared storage center |
...
| Check | Threshold | Action if Failed |
|---|---|---|
| Count edge crossings | Max 5 total, 0 on critical paths | Restructure layout. Add spatial separation. Consolidate edges. |
| Critical path isolation | For EACH flow type (write/read/sync): trace the path. Does your eye hit a line of a different color? | If YES → FAIL. Spatially separate flow types. |
| Bottom-half independence | Cover top 40%. Is bottom 60% readable on its own? | If NO → FAIL. The diagram is bottom-heavy. Redistribute or simplify. |
| Whitespace between layers | Visible gap between every layer container | If layers touch or lines blur into layer borders → FAIL. Add padding. |
| Label anchoring | Every flow label is attached to its edge, not floating | If any label floats → fix edge label placement |
| Figure-ground contrast | Layer backgrounds are visibly distinct from canvas | If layers disappear into background → increase fill opacity by 20%+ |
...
| Split layout (write-left, read-right) | 20 | Dedicated corridors per flow type |

**Enforcement:** Count edges in `.d2` source before rendering. If over budget, apply Rule 22 or switch to a wider layout.
